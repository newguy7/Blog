...
# CONFIGURE TABLES
# BACKPOPULATES WILL NEVER SHOW UP AS COLUMNS IN SQLITE
...
def register():
    ...
    if register_form.validate_on_submit():     
        ...
        salted_password_hash = generate_password_hash(register_form.password.data , method='pbkdf2:sha256', salt_length=8)                             
        user = User(email=email,name=name,password=salted_password_hash)
        db.session.add(user)
        db.session.commit()
...
def show_post(post_id):    
    ...
    if form.validate_on_submit():
        new_comment = Comment(
            comment_body = form.comment_text.data,
        ...
        db.session.commit()        
    return render_template("post.html", time=comment_time,post=requested_post, logged_in = current_user.is_authenticated,current_user=current_user, form=form)
